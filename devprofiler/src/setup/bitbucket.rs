...
fn save_auth_info_to_db(auth_info: &mut AuthInfo) {
    let db = get_db();
    let now = SystemTime::now();
    let since_epoch = now.duration_since(UNIX_EPOCH).expect("Time went backwards");  
    auth_info.timestamp = Some(since_epoch.as_secs());
